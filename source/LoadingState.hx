...
class LoadingState extends MusicBeatState
{
	...
	function initializeCallbacks() callbacks = new MultiCallback(onLoadComplete);
	...
	function loadLibraryAssets() {
		...
		var weekLibrary:String = (PlayState.storyWeek > 0) ? "week" + PlayState.storyWeek : "tutorial";
	...
	function checkLibrary(library:String) {
		if (Assets.getLibrary(library) == null) {
			@:privateAccess if (!LimeAssets.libraryPaths.exists(library)) {
	...
	function cleanupAudio() if (stopMusic && FlxG.sound.music != null) FlxG.sound.music.stop();
	
	function switchToTargetState() FlxG.switchState(() -> target);
	
	static function getSongPath():String return Paths.inst(PlayState.SONG.song);
	
	static function getVocalPath():String return Paths.voices(PlayState.SONG.song);
	
	inline static public function loadAndSwitchState(target:FlxState, stopMusic = false) FlxG.switchState(() -> getNextState(target, stopMusic));
	
	static function getNextState(target:FlxState, stopMusic = false):FlxState {
		Paths.setCurrentLevel("week" + PlayState.storyWeek);
		if (!areRequiredAssetsLoaded()) return new LoadingState(target, stopMusic);
		if (stopMusic && FlxG.sound.music != null) FlxG.sound.music.stop();
		return target;
	}
	
	static function areRequiredAssetsLoaded():Bool
		return isSoundLoaded(getSongPath()) && (!PlayState.SONG.needsVoices || isSoundLoaded(getVocalPath())) && isLibraryLoaded("shared");
	...
	static function loadSongsManifest(id:String):Future<AssetLibrary> {
		var promise:Promise<AssetLibrary> = new Promise<AssetLibrary>();
		var pathInfo:{path:String, rootPath:String} = getLibraryPathInfo(id);
	...
	static function getLibraryPathInfo(id:String):{path:String, rootPath:String} {
		...
		if (libraryPaths.exists(id)) {
			var path:Null<String> = libraryPaths[id];
			return {path: path, rootPath: Path.directory(path)};
		} else {
			var path:String = id;
			var rootPath:String = StringTools.endsWith(path, ".bundle") ? path : Path.directory(path);
			if (StringTools.endsWith(path, ".bundle")) path += "/library.json";
			@:privateAccess
...
class MultiCallback
{
	...
	public function add(id:String = "untitled"):Void->Void {
		var callbackId:String = '$length:$id';
		length++;
		numRemaining++;
		...
		var func:Void->Void = function() {
			...
			else log('already fired $callbackId');
		}
	...
	inline function log(msg:String):Void if (logId != null) trace('$logId: $msg');
